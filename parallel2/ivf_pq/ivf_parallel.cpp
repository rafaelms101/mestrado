...
void parallel_search (int nsq, int last_search, int my_rank, int last_aggregator, int k, int last_assign, char* arquivo){
	...
	while(1){
		vetor=(int*)realloc(vetor,sizeof(int)*(entrou+1));
		coaidx=(int*)realloc(coaidx,sizeof(int)*(entrou+1));
		residual=(mat*)realloc(residual,sizeof(mat)*(entrou+1));
		residual[entrou].n=1;

		MPI_Irecv(&vetor[entrou], 1, MPI_INT, MPI_ANY_SOURCE, ASSIGN, MPI_COMM_WORLD, &request);
	...
	for(int i=0; i<entrou;i++){
		...
		q[i]=ivfpq_search(ivf, residual[i], ivfpq.pq, coaidx[i]);
		/*
	...
	for(int i=0; i<entrou;i++){
		dest=vetor[i]%(last_aggregator-last_search)+last_search+1;
		MPI_Send(&q[i].idx.n, 1, MPI_INT, dest, SEARCH, MPI_COMM_WORLD);
		MPI_Send(&q[i].idx.mat[0], q[i].idx.n, MPI_INT, dest, SEARCH, MPI_COMM_WORLD);
		MPI_Send(&q[i].dis.mat[0], q[i].idx.n, MPI_FLOAT, dest, SEARCH, MPI_COMM_WORLD);
	}
